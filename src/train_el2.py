...
def train():
  ...
  y_tilde_e2, likelihoods_e2, entropy_bottleneck_e2 = entropy_estimation_e2(y_e2, True)
  x_tilde_e2 = synthesis_transform_e2(y_tilde_e2, args.conv_filters, args.num_filters*2)
  x_rec = x_tilde + x_tilde_e1 + x_tilde_e2
  ...
  with tf.name_scope("distortion"):
    #train_mse = tf.reduce_sum(tf.squared_difference(x - x_tilde - x_tilde_e1, x_tilde_e2))
  ...
  with tf.name_scope("rate_distortion_loss"):
    train_loss =  train_mse + args.lmbda * train_bpp_e2
  ...
  tf.add_to_collection('e2_pre256', comp_vars_e2)
  print(comp_vars_e2)

  # Minimize loss and auxiliary loss, and execute update op.
  step = tf.train.create_global_step()
  main_optimizer = tf.train.AdamOptimizer(learning_rate=1e-5)
  main_step = main_optimizer.minimize(train_loss, global_step=step,
                                      var_list=[var for var in comp_vars_e2])

  aux_learning_rate = tf.train.exponential_decay(0.01, step, 1000, 0.96, staircase=False)
  aux_optimizer = tf.train.AdamOptimizer(learning_rate=aux_learning_rate)
  aux_step = aux_optimizer.minimize(entropy_bottleneck_e2.losses[0], global_step=step,
                                    var_list=[var for var in comp_vars_e2])

  train_op = tf.group(main_step, aux_step, entropy_bottleneck_e2.updates[0])
  ...
  with tf.Session() as sess:
    ...
    with tf.train.MonitoredTrainingSession(
        hooks=hooks, checkpoint_dir=args.e2_checkpoint_dir)as sess:
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--lambda", type=float, default=300, dest="lmbda",
      help="Lambda for rate-distortion tradeoff.")
  parser.add_argument(
      "--last_step", type=int, default=50000000,
