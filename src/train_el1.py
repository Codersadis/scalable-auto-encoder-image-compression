...
def train():
  ...
  with tf.name_scope("rate_distortion_loss"):
    train_loss =  train_mse + args.lmbda * train_bpp_e1
  ...
  comp_vars_e1   = [var for var in tf.all_variables() if var not in comp_vars_base]
  print(comp_vars_e1)
  ...
  main_step = main_optimizer.minimize(train_loss, global_step=step,
                                      var_list=[var for var in comp_vars_e1])

  aux_learning_rate = tf.train.exponential_decay(0.001, step, 1000, 0.96, staircase=False)
  aux_optimizer = tf.train.AdamOptimizer(learning_rate=aux_learning_rate)
  aux_step = aux_optimizer.minimize(entropy_bottleneck_e1.losses[0], global_step=step,
                                    var_list=[var for var in comp_vars_e1])

  train_op = tf.group(main_step, aux_step, entropy_bottleneck_e1.updates[0])
  ...
  with tf.Session() as sess:
    ...
    with tf.train.MonitoredTrainingSession(
        hooks=hooks, checkpoint_dir=args.e1_checkpoint_dir)as sess:
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--e1_checkpoint_dir", default="color/error500",
      help="Directory where to save/load e1 layer model checkpoints.")
  parser.add_argument(
      "--base_checkpoint_dir", default="color/base500_feature48",
  ...
  parser.add_argument(
      "--last_step", type=int, default=500000,
