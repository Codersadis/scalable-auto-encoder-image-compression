...
def train():
  ...
  y_tilde_e3, likelihoods_e3, entropy_bottleneck_e3, string_e3 = entropy_estimation_e3(y_e3, False)
  x_tilde_e3 = synthesis_transform_e3(y_tilde_e3, args.conv_filters, args.num_filters * 2)

  x_en4 = x - x_tilde - x_tilde_e1 - x_tilde_e2 - x_tilde_e3
  y_e4 = analysis_transform_e4(x_en4, args.conv_filters, args.num_filters * 2)
  y_tilde_e4, likelihoods_e4, entropy_bottleneck_e4 = entropy_estimation_e4(y_e4, True)
  x_tilde_e4 = synthesis_transform_e4(y_tilde_e4, args.conv_filters, args.num_filters * 2)
  x_rec = x_tilde + x_tilde_e1 + x_tilde_e2 + x_tilde_e3 + x_tilde_e4
  ...
  with tf.name_scope("rate"):
    ...
    train_bpp_e4 = tf.reduce_sum(tf.log(likelihoods_e4)) / (-np.log(2) * num_pixels)
  ...
  with tf.name_scope("rate_distortion_loss"):
    train_loss =  train_mse + args.lmbda * train_bpp_e4
  ...
  tf.add_to_collection('e4_pre256', comp_vars_e4)
  print(comp_vars_e4)
  ...
  main_step = main_optimizer.minimize(train_loss, global_step=step,
                                      var_list=[var for var in comp_vars_e4])
  ...
  aux_step = aux_optimizer.minimize(entropy_bottleneck_e4.losses[0], global_step=step,
                                    var_list=[var for var in comp_vars_e4])

  train_op = tf.group(main_step, aux_step, entropy_bottleneck_e4.updates[0])
  ...
  logged_tensors = [
      ...
      tf.identity(train_bpp_e4, name="train_bpp_e4"),
  ...
  saver_e4 = tf.train.Saver(comp_vars_e4)
  ...
  with tf.Session() as sess:
    ...
    enhance4_ckpt = tf.train.get_checkpoint_state(args.e4_checkpoint_dir)
      ...
      if enhance_ckpt and enhance_ckpt.model_checkpoint_path:
        ...
        if enhance2_ckpt and enhance2_ckpt.model_checkpoint_path:
          ...
          if enhance3_ckpt and enhance3_ckpt.model_checkpoint_path:
            ...
            if enhance4_ckpt and enhance4_ckpt.model_checkpoint_path:
              saver_e4.restore(sess, tf.train.latest_checkpoint(args.e4_checkpoint_dir))
              print("Restore the model from {}.".format(enhance4_ckpt.model_checkpoint_path))

    with tf.train.MonitoredTrainingSession(
        hooks=hooks, checkpoint_dir=args.e4_checkpoint_dir)as sess:
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      "--e4_checkpoint_dir", default="color/enhance4",
